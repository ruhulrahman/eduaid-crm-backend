<template>
  <div class="section-wrapper">
    ...
    <b-breadcrumb>
      ...
      <b-breadcrumb-item active>Status Groups</b-breadcrumb-item>
    </b-breadcrumb>
      <div class="form-wrapper">
      <b-card title="Status Groups Search">
          <b-card-text>
  ...
  <b-card class="mt-3">
    <b-card-title>
      <b-row>
        <b-col>
          <h4 class="card-title mb-0 pl-0">Status Group List</h4>
        </b-col>
              ...
              <table class="table table-striped table-hover table-bordered">
                ...
                <tbody v-for="(item, index) in listData" :key="index">
                  <tr style="font-size: 12px;">
                    ...
                    <td class="text-left">
                        <router-link :to="`/status-groups/${hash_id(item.id)}`"><span>{{ item.name }}</span></router-link>
                    </td>
                    <td class="text-left">
                      {{ item.code }}
                    </td>
                    ...
                    <td class="text-center">
                      ...
                      <router-link :to="`/status-groups/${hash_id(item.id)}`" v-tooltip="'Add Status'" v-if="has_permission('permitted')" class="action-btn active" style="width: 20px !important; height: 20px !important; font-size:12px"><i class="ri-add-fill"></i></router-link>
                    </td>
                  </tr>
                </tbody>
              </table>
    ...
    <b-modal id="modal-1" ref="editModal" size="lg" title="Status Groups" centered :hide-footer="true">
...
export default {
  ...
  methods: {
    ...
    async getListData () {
      ...
      var result = await RestApi.getData(baseURL, 'api/v1/admin/ajax/get_status_group_list', params)
      if (result.success) {
    ...
    async toggleActiveStatus (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/toggle_status_group_active_status', item)
      this.loading = false
      if (result.success) {
    ...
    async deleteConfirmed (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/delete_status_group_data', item)
      this.loading = false
      if (result.success) {
        this.$toast.success({
          title: 'Success',
